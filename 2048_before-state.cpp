...
class pattern : public feature {
 ...
 public:
  ...
  virtual float update(const board& b, float u) {
    ...
    for (int i = 0; i < iso_last; i++) {
      size_t index = indexof(isomorphic[i], b);
      operator[](index) += u_split; // update
      value += operator[](index); // estimate
    }
    return value;
  }
 ...
 public:
  ...
  state select_best_move(const board& b) const {
    ...
    for (state* move = after; move != after + 4; move++) {
      if (move->assign(b)) {
        ...
        move->set_value(move->reward() + value_);
  ...
  void update_episode(std::vector<state>& path, float alpha = 0.1) const {
    ...
    for ( ; path.size(); path.pop_back()) {
      ...
      // exact = estimate(move.before_state());
    }
  }
...
int main(int argc, const char* argv[]) {
  ...
  tdl.add_feature(new pattern({ 0, 1, 4, 7, 10, 13 }));
  ...
  for (size_t n = 1; n <= total; n++) {
    ...
    while (true) {
      ...
      path.push_back(best); // QUESTION: save best into path through push_back, all (s,s',a,r) are saved in path?
